...
const TopBar: React.FC<TopBarProps> = ({ activeTab, setActiveTab, currentUser, onLogout, onToggleSidebar }) => {
  ...
  const getRoleDisplayName = (role: string, id: string) => {
    if (id === 'ADM-001') return 'ADM-001';
    switch (role) {
      case 'OPERADOR_SALA': return 'Coordenação';
      case 'ANALISTA_RISCO': return 'Analista Risco';
      case 'AMBULANCIA': return 'Unidade Móvel';
      case 'EMPRESA_CLIENTE': return 'Corporativo';
      default: return role;
    }
  };
      ...
      <div className="h-16 flex items-center justify-between">
        <div className="flex items-center gap-4 md:gap-10">
          ...
          <nav className="hidden lg:flex gap-6">
            ...
            {['ADMIN_OC', 'OPERADOR_SALA'].includes(currentUser.role) && (
              <button 
            ...
            {['ADMIN_OC', 'ANALISTA_RISCO'].includes(currentUser.role) && (
              <button 
