...
const AccountManagement: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleOpenCreate = () => {
    setFormData({
      name: '', email: '', phone: '', role: 'OPERADOR_SALA', 
  ...
  const handleDelete = (userId: string) => {
    if (window.confirm(`Tem a certeza que deseja ELIMINAR permanentemente a conta ${userId}? Esta ação é irreversível.`)) {
      ...
      auditLogger.log({id: 'ADM-001', name: 'Marina Sengo', role: 'ADMIN_OC'}, 'DATA_EXPORT_PDF', 'ADMIN', `Eliminação de conta: ${userId}`);
    }
  };
  ...
  const handleToggleStatus = (user: AdminUser) => {
    ...
    if (window.confirm(`Deseja ${action} o acesso de ${user.name}?`)) {
      ...
      auditLogger.log({id: 'ADM-001', name: 'Marina Sengo', role: 'ADMIN_OC'}, 'DATA_EXPORT_PDF', 'ADMIN', `${action} conta: ${user.id}`);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (view === 'create') {
        ...
        auditLogger.log({id: 'ADM-001', name: 'Marina Sengo', role: 'ADMIN_OC'}, 'DATA_EXPORT_PDF', 'ADMIN', `Nova conta criada: ${newId}`);
      } else {
        setAccounts(prev => prev.map(a => a.id === selectedUser?.id ? { ...a, ...formData } : a));
        auditLogger.log({id: 'ADM-001', name: 'Marina Sengo', role: 'ADMIN_OC'}, 'DATA_EXPORT_PDF', 'ADMIN', `Conta modificada: ${selectedUser?.id}`);
      }
  ...
  const getRoleBadge = (role: string) => {
    switch(role) {
      case 'ADMIN_OC': return 'bg-purple-100 text-purple-700 border-purple-200';
      case 'OPERADOR_SALA': return 'bg-blue-100 text-blue-700 border-blue-200';
      case 'AMBULANCIA': return 'bg-red-100 text-red-700 border-red-200';
      case 'EMPRESA_CLIENTE': return 'bg-emerald-100 text-emerald-700 border-emerald-200';
    ...
    <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[700px] animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        {view === 'list' ? (
            ...
            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-4 flex-1 min-w-[300px]">
                ...
                >
                  ...
                  <option value="ADMIN_OC">Administradores</option>
                  <option value="OPERADOR_SALA">Coordenação</option>
                  <option value="AMBULANCIA">Ambulâncias</option>
                  <option value="EMPRESA_CLIENTE">Empresas</option>
                </select>
              </div>
              <button 
          ...
          /* VISTA DE FORMULÁRIO (CRIAR/EDITAR) */
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <option value="OPERADOR_SALA">Operador de Coordenação</option>
                        <option value="ANALISTA_RISCO">Gestor de Risco</option>
                        <option value="AMBULANCIA">Unidade de Ambulância</option>
                        <option value="EMPRESA_CLIENTE">Gestor de Cliente</option>
                        <option value="ADMIN_OC">Administrador Local</option>
                      </select>
                    </div>
                    <div className="space-y-2">
