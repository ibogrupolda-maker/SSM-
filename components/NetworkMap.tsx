...
import { EmergencyCase, Employee, Company } from '../types';
import { EMPLOYEES, COMPANIES } from '../constants';
...
const NetworkMap: React.FC<NetworkMapProps> = ({ incidents, hideSidebar = false }) => {
  ...
  const providers = [
    { 
      id: 'LENMED-01', 
      type: 'hospital', 
      pos: [-25.952, 32.598] as [number, number], 
      label: 'Lenmed Maputo Private Hospital', 
      address: 'Sommerschield II, Maputo',
      phone: '+258 84 303 0967',
      status: 'Operacional', 
      province: 'Maputo' 
    },
    { 
      id: 'ICOR-01', 
      type: 'hospital', 
      pos: [-25.960, 32.590] as [number, number], 
      label: 'ICOR - Instituto do Coração', 
      address: 'Av. Kenneth Kaunda, nº 1111, Maputo',
      phone: '+258 84 327 4800',
    ...
    { 
      id: 'HCM-01', 
      type: 'hospital', 
      pos: [-25.975, 32.585] as [number, number], 
      label: 'Hospital Central de Maputo', 
      address: 'Av. Eduardo Mondlane, Maputo',
      phone: '+258 21 357 900',
      status: 'Referência Nacional', 
      province: 'Maputo' 
    },
    { 
      id: 'CLINICARE-01', 
      type: 'hospital', 
      pos: [-25.965, 32.582] as [number, number], 
      label: 'Clinicare – Clínica Privada de Maputo', 
      address: 'Rua Ângelo Azarias Chichava 50, Maputo',
      phone: '+258 82 000 3344',
      status: 'Disponível', 
      province: 'Maputo' 
    },
    { id: 'AMB-MZ-01', type: 'ambulance', pos: [-25.965, 32.575] as [number, number], label: 'Unidade Móvel Maputo 01', status: 'Disponível', province: 'Maputo' },
    { id: 'AMB-MZ-02', type: 'ambulance', pos: [-25.970, 32.565] as [number, number], label: 'Unidade Móvel Maputo 02', status: 'Ocupado', province: 'Maputo' },
    { id: 'HCB-01', type: 'hospital', pos: [-19.8317, 34.8367] as [number, number], label: 'Hospital Central da Beira', status: 'Operacional', province: 'Sofala' },
    { id: 'HCN-01', type: 'hospital', pos: [-15.1165, 39.2662] as [number, number], label: 'Hospital Central de Nampula', status: 'Operacional', province: 'Nampula' },
    { id: 'HPT-01', type: 'hospital', pos: [-16.1564, 33.5867] as [number, number], label: 'Hospital Provincial de Tete', status: 'Operacional', province: 'Tete' },
  ];
  ...
  useEffect(() => {
    ...
    incidents.forEach(inc => {
      ...
      const color = inc.status === 'active' || inc.status === 'triage' ? 'red' : 'blue';
      const icon = L.divIcon({
        className: 'custom-marker',
        html: `
          <div class="relative flex items-center justify-center">
            <div class="marker-pulse bg-${color}-500 opacity-20"></div>
            <div class="z-10 bg-${color}-600 p-1.5 rounded-full shadow-lg border-2 border-white text-white flex items-center justify-center transition-transform hover:scale-110">
