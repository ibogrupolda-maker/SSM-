...
const ResourceManagement: React.FC<ResourceManagementProps> = ({ incidents }) => {
  const [view, setView] = useState<'map' | 'grid' | 'list'>('map');

  const stats = [
    { label: 'Total de Recursos', value: '8', icon: Package, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Disponíveis', value: '4', icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Atribuídos', value: '2', icon: Clock, color: 'text-orange-600', bg: 'bg-orange-50' },
    { label: 'Offline', value: '2', icon: Activity, color: 'text-red-600', bg: 'bg-red-50' },
  ];
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-6">
        ...
        <div className="h-[600px] shadow-2xl shadow-slate-200/50 rounded-[3rem] overflow-hidden border border-slate-200 relative">
          <NetworkMap incidents={incidents} />
